...
In the domain of natural language processing (NLP), a comprehensive analysis was conducted on the State of the Nation Addresses (SONA) by South African presidents from 1994 to 2023, employing advanced emotion and theme extraction techniques. This study utilized sentiment analysis, leveraging two distinct lexicons—AFINN and Bing—to gauge the polarity of emotions within the speeches. Concurrently, five sophisticated topic modeling algorithms were applied, namely Latent Semantic Analysis (LSA), Probabilistic Latent Semantic Analysis (pLSA), Latent Dirichlet Allocation (LDA), Correlated Topic Model (CTM), and Author-Topic Model (ATM), to distill and categorize the predominant themes.

The empirical findings indicated a lack of significant divergence between the AFINN and Bing lexicons in mapping the sentiment trajectory—both positive and negative—across different time periods and presidencies. It was observed that the specific lexicon words contributing to these sentiments varied across the speeches, yet no discernible pattern emerged when analyzed by presidential term. In terms of thematic consistency, the LSA, pLSA, LDA, and CTM exhibited remarkable uniformity in identifying key governance-related themes encompassing economic, social, and structural dimensions, as well as contextual contemporary issues like pandemics and sports.
However, the application of the ATM revealed a nuanced differentiation, unearthing more personalized thematic elements aligned with individual presidential agendas. This model demonstrated an enhanced capability in distinguishing topics more effectively across different presidencies, underscoring the utility of ATM in extracting distinct, president-specific themes from the corpus of SONA speeches. This study underscores the potency of NLP techniques in thematic and sentiment analysis, particularly in the context of political discourse, offering valuable insights into the subtle variations in thematic emphasis and sentiment across different administrative tenures.
...
<b><u> Data pre-processing </b></u>


In the realm of textual data analysis, a series of intricate preprocessing procedures is paramount to prepare and refine the dataset for subsequent analytical scrutiny. Initially, an extraction process is undertaken wherein a ZIP archive, comprising multiple text documents, is meticulously opened, and its contents are methodically transferred to a designated directory. This is followed by an intricate file filtering stage, during which the directory's contents are scrupulously sifted, retaining only those files with a '.txt' extension, thereby ensuring format uniformity.
Subsequent to these preliminary stages, a meticulous process of data reading and parsing is executed. Each text file is individually accessed; its inaugural line, postulated to contain the date, is extracted and segregated. The residual contents of each file are then assiduously read and conserved. In the ensuing phase, a DataFrame is constructed, astutely incorporating filenames, the previously extracted dates, and the comprehensive speech text.
A critical phase of metadata extraction ensues, wherein, through precise string operations, pivotal information such as the year and the president's name are discerned from the filenames and adroitly integrated as discrete columns within the DataFrame. The text cleaning phase plays a pivotal role, involving the removal of URLs, HTML entities, and newline characters from the speeches, and employing periods and question marks as delimiters to fragment the text into sentences. This stage further includes the flattening of nested lists and the exclusion of null strings.
The data export phase involves the systematic saving of the full speeches and the segmented sentences into two distinct CSV files. For enhanced analytical precision, the CSV files are reimported into DataFrames, and a lemmatization function, tailored to convert words into their base form using appropriate part-of-speech tags, is defined and applied. This phase also involves the exclusion of stop words and a custom list of additional words, with a stringent focus on retaining only those words recognized as English.
The final adjustments and export phase encompass the application of two separate cleaning functions to the speeches: one excluding additional words and the other retaining them. Subsequently, the refined speeches and sentences are saved into new CSV files. In a concluding act of data optimization, the original 'speech' column is meticulously dropped from the sentences DataFrame prior to the final export, thereby streamlining the dataset for advanced analysis.
...
<b><u>  Latent Dirichlet Allocation (LDA) </b></u>
...
![tf-idf: Topic 1 ~ economic and social governance](lsa_plots/sentences_tfidf_topic_1.png){#fig-sentences-tfidf-topic1}

![BoW: Topic 2 ~ governmental structures](lsa_plots/sentences_bow_topic_2.png){#fig-sentences-bow-topic2}

![tf-idf: Topic 2 ~ gratitude and reformation](lsa_plots/sentences_tfidf_topic_2.png){#fig-sentences-tfidf-topic2}
...
![BoW: Topic 1 ~ structural governance](plsa_plots/words-BoW_topic_1.png){#fig-pLSA-words-bow-topic1}

![tf-idf: Topic 1 ~ pandemic and resources](plsa_plots/words-tf-idf_topic_1.png){#fig-pLSA-words-tfidf-topic1}

![BoW: Topic 2 ~ social governance](plsa_plots/words-BoW_topic_2.png){#fig-pLSA-words-bow-topic2}

![tf-idf: Topic 2 ~ prosperity](plsa_plots/words-tf-idf_topic_2.png){#fig-pLSA-words-tfidf-topic2}

![BoW: Topic 3 ~ economic governance](plsa_plots/words-BoW_topic_3.png){#fig-pLSA-words-bow-topic3}

![tf-idf: Topic 3 ~ resolution](plsa_plots/words-tf-idf_topic_3.png){#fig-pLSA-words-tfidf-topic3}

![BoW: Topic 4 ~ resource governance](plsa_plots/words-BoW_topic_4.png){#fig-pLSA-words-bow-topic4}

![tf-idf: Topic 4 ~ sport and support](plsa_plots/words-tf-idf_topic_4.png){#fig-pLSA-words-tfidf-topic4}

![BoW: Topic 5 ~ structural and economic governance](plsa_plots/words-BoW_topic_5.png){#fig-pLSA-words-bow-topic5}

![tf-idf: Topic 5 ~ sport and economy](plsa_plots/words-tf-idf_topic_5.png){#fig-pLSA-words-tfidf-topic5}

Topic plots for pLSA tokenized by words and executed within a BoW (left) and tf-idf (right) framework. 
:::


Again, like in LSA, the coherence scores oscillate across a topic-number range when using sentence-tokenized speeches for pLSA. For both corpus approaches, the first peak is reached at four topics, as rendered in @fig-pLSA-sentences-coherence. 
...
From @fig-pLSA-sentences-topics, some slight similarity in topic type compared to the word-tokenized speeches is seen, specifically that of governance facets. Structural and social governance re-appear as the second and fourth topics for the BoW-related corpus, respectively. Whilst, economic governance is again seen for the tf-idf-related corpus, either combined with resource governance (topic one) or growth (topic four). Governmental structures, previously described as a topic part of the sentence-tokenized LSA implementation, also appears here again.  Evidently, there is some topic correspondence across the different tokenized corpora and different topic modelling approaches. 

::: {#fig-pLSA-sentences-topics layout-ncol=2}

![BoW: Topic 1 ~ governance growth](plsa_plots/sentences-BoW_topic_1.png){#fig-pLSA-sentences-bow-topic1}

![tf-idf: Topic 1 ~ economic and resource governance](plsa_plots/sentences-tf-idf_topic_1.png){#fig-pLSA-sentences-tfidf-topic1}

![BoW: Topic 2 ~ structural governance](plsa_plots/sentences-BoW_topic_2.png){#fig-pLSA-sentences-bow-topic2}

![tf-idf: Topic 2 ~ governmental structures](plsa_plots/sentences-tf-idf_topic_2.png){#fig-pLSA-sentences-tfidf-topic2}

![BoW: Topic 3 ~ social collectives in education](plsa_plots/sentences-BoW_topic_3.png){#fig-pLSA-sentences-bow-topic3}

![tf-idf: Topic 3 ~ overall governance](plsa_plots/sentences-tf-idf_topic_3.png){#fig-pLSA-sentences-tfidf-topic3}

![BoW: Topic 4 ~ social governance](plsa_plots/sentences-BoW_topic_4.png){#fig-pLSA-sentences-bow-topic4}

![tf-idf: Topic 4 ~ economic governance and growth](plsa_plots/sentences-tf-idf_topic_4.png){#fig-pLSA-sentences-tfidf-topic4}
...
A systematic hyperparameter grid search was conducted, spanning $\alpha$ values from 0.1 to 1 in increments of 0.1 and $\beta$ values from 0.1 to 1 in increments of 0.2, with the objective of determining the optimal combination for coherence score maximization across a topic range of two to ten. This empirical approach revealed, through the application of contour plots to both word-and-sentence tokenized speeches (@fig-LDA-contours), a distinct pattern: the coherence score remained largely unaffected by variations in the $\beta$ parameter, yet exhibited a pronounced sensitivity to alterations in the $\alpha$ parameter.
...
Examination of Table 1, presenting a subset of the grid search results for word-tokenized speeches, further corroborated these findings. The values therein demonstrated a tendency for higher $\alpha$ values to consistently yield maximum coherence scores, irrespective of the $\beta$ value. This table also highlighted the superiority of the Bag-of-Words (BoW) corpus over the term frequency-inverse document frequency (tf-idf) corpus in achieving optimal hyperparameter settings, particularly with a topic count of nine. Parallel results were observed in Table 2, where the employment of high alpha values in conjunction with the BoW corpus, irrespective of beta values, resulted in the most favorable coherence scores, even with a minimal topic count of two.
...
The employment of pyLDAvis for interactive visualization of LDA results yielded insightful revelations. The left panel of this visualization tool, presenting an inter-topic distance map, offered a comprehensive view of the topic distribution and interrelations, employing multi-dimensional scaling (MDS) to project these relationships onto a two-dimensional plane. The size of each topic circle in this map was directly proportional to its prevalence, while the right panel provided a horizontal bar chart delineating the most salient terms for each topic, thereby facilitating a deeper understanding of the topic's essence. The juxtaposition of corpus-wide frequencies (light-blue bars) against topic-specific frequencies (red bars) for each term allowed for an intricate exploration of topic-term relationships.

 
Further analytical depth was achieved through the application of the relevance metric, which leveraged a weight parameter lambda to balance the probability of a word within a topic against its lift—a measure of the word's frequency within a topic relative to its marginal probability across the corpus. This metric proved instrumental in distinguishing between terms that were prevalent yet not exclusive to a topic (high lambda values) and those that were rare but highly indicative of a topic (low lambda values).

When applied to the word-tokenized BoW corpus, the LDA results revealed several distinct thematic clusters. For instance, at a lambda value of 0.1, Topic 1, characterized by terms like “mining” and “alliance,” was identified as the most prevalent, suggesting a focus on mining-related activities. In contrast, Topic 9, with a significantly smaller presence, was associated with local governance themes like “municipality” and “refurbishment.” This thematic diversity was further reflected in the spatial distribution of topics on the inter-topic distance map, where topics related to economic activities (Topics 1 and 4) were shown to have a certain degree of proximity, indicative of underlying thematic similarities, while Topic 2, centered around abstract concepts like “unity” and “compatriots,” was markedly distant from other topics, underscoring its unique thematic focus.
...
In the case of the sentence-tokenized BoW corpus, the two discerned topics exhibited a marked separation, suggesting minimal thematic overlap or interrelation, thereby underscoring the distinct nature of the topics identified through this corpus-specific LDA implementation.
...
When accounting for potential correlation between topics,  @fig-ctm-coherence-sentences (for sentence-tokenized speeches) and @fig-ctm-coherence-words (for word-tokenized speeches) indicate that the optimal number of topics for a BoW-related corpus is three and two, respectively. 
...
![Sentences: Topic 1 ~ economy](ctm_plots/sentences_1.png){#fig-ctm-sentences-topic1}

![Words: Topic 1 ~ economic and social governance](ctm_plots/words_1.png){#fig-ctm-words-topic1}

![Sentences: Topic 2 ~ economic, social, and structural governance](ctm_plots/sentences_2.png){#fig-ctm-sentences-topic2}

![Words: Topic 2 ~ societal collectives](ctm_plots/words_2.png){#fig-ctm-words-topic2}

![Sentences: Topic 3 ~ societal collectives](ctm_plots/sentences_3.png){#fig-ctm-sentences-topic3}

Topic plots where either sentences (left) or words (right) were used as tokens for CTM application. 
:::
...
As for CTM, the coherence plots presented in @fig-ATM-coherence,  suggest that three and two topics relay the maximum coherence score for the BoW-related corpus for the respective sentence-tokenized and word-tokenized speeches. 
...
df = pd.DataFrame(columns=['President', 'Document IDs', 'Word-Based Topics', 'Sentence-Based Topics'])
...
for president in presidents:
    ...
    rows.append({'President': president, 'Document IDs': doc_ids, 
...
for idx, row in df.iterrows():
    ...
    df.at[idx, 'Document IDs'] = ', '.join(map(str, row['Document IDs']))
